...
# GitHub'daki klasör isimleriyle (şablonlar / statik) birebir eşleştirildi:
app = Flask(__name__, template_folder='şablonlar', static_folder='statik')

# Render PostgreSQL bağlantı adresini standart formata çevirme
...
UPLOAD_FOLDER = os.path.join('statik', 'uploads')
...
# ==========================================
# MERKEZİ VERİ PAKETİ VE SOKET YAYINI
# ==========================================
def tum_durum_verisi_olustur():
...
def tum_durumu_yayinla():
    veri = tum_durum_verisi_olustur()
    socketio.emit('tam_durum_guncelle', veri)
...
def durum_getir():
    return jsonify(tum_durum_verisi_olustur())
...
def urun_ekle():
    ...
    for dosya in dosyalar:
        if dosya and dosya.filename:
            ...
            url = f"/statik/uploads/{dosya.filename}"
...
def handle_connect():
    ...
    socketio.emit('tam_durum_guncelle', tum_durum_verisi_olustur(), to=request.sid)
