        ...
        with app.app_context():
            if aktif_urun_id:
                ...
                if urun:
                    ...
                    # Satılan ürünün ön tekliflerini otomatik temizle
...
def durum_getir():
    ...
    try:
        ...
        # Satılmış ürünlerin ön teklifleri arşive yansıtılmaz
        on_teklifler = []
        try:
...
def on_teklif_ver():
    ...
    # EN YÜKSEK TABAN FİYATI BELİRLE (Açılış, Canlıdaki Anlık Pey, Önceki Ön Teklifler)
    ...
    # %10 (Minimum 100 TL) Artış Kuralı
    ...
    # Ürün pey görmüşse veya ön teklif verilmişse taban fiyattan en az %10 fazla olmalıdır
...
def excel_indir():
    try:
        ...
        with pd.ExcelWriter(dosya_yolu, engine='openpyxl') as writer:
            ...
            if musteriler:
                df_mus = [{"Ad Soyad": m.ad_soyad, "Telefon": m.telefon, "E-posta": m.email, "Adres": m.adres, "Durum": m.durum} for m in musteriler]
