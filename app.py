...
# ==========================================
app = Flask(__name__)
app.config['SECRET_KEY'] = 'halic_hamid_antik_mezat_gizli_anahtar_1453'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///halic_mezat.db'
...
class Urun(db.Model):
    ...
    durum = db.Column(db.String(20), default="Aktif")  # Aktif, Sayim, Satıldı
...
class OdemeKaydi(db.Model):
    ...
    tutar = db.Column(db.Float, nullable=False)
    durum = db.Column(db.String(50), default="odendi")
    tarih = db.Column(db.DateTime, default=datetime.utcnow)
...
def puan_cezasi_uygula(kullanici_id, ceza_puani):
    """Kullanıcının puanını düşürmek için kullanılır."""
    kullanici = Kullanici.query.get(kullanici_id)
    if kullanici:
        kullanici.puan = max(0.0, kullanici.puan - ceza_puani)
        db.session.commit()
        return True
    return False
...
def durum_getir():
    ...
    try:
        ...
        teklifler = Teklif.query.order_by(Teklif.id.desc()).all()
        musteri_pey_listesi = []
        for t in teklifler:
            u = Urun.query.get(t.urun_id)
            musteri_pey_listesi.append({
                "urun_id": t.urun_id,
                "lot": u.lot_no if u else "-",
                "urun_adi": u.urun_adi if u else "Silinmiş Ürün",
                "musteri_adi": t.musteri_adi or "Bilinmeyen",
                "fiyat": t.tutar or 0.0
            })
        ...
        for p in musteri_pey_listesi:
            ...
            gruplanmis_dosyalar[m_adi]["toplam_tutar"] += float(p.get('fiyat', 0.0))
...
def kayit_ol():
    ...
    if kullanici:
        ...
        kullanici = Kullanici(ad_soyad=ad, telefon=tel, email=mail, adres=adres)
...
def pey_ver():
    ...
    if islem == 'hemen_al':
        ...
        if kullanici:
            kullanici.bonus += hemen_al_fiyat * 0.05
...
def odeme_yap():
    ...
    # Sanal POS (Paytr / iyzico vb.) Başarılı Dönüş Simülasyonu
    yeni_odeme = OdemeKaydi(
    ...
    # Müşteri Günlük Alım Puanlaması (%100 Üzerinden Hesaplama)
    kullanici = Kullanici.query.get(kullanici_id)
    guncel_puan = 100.0
    if kullanici:
        kullanici.puan = min(100.0, (kullanici.puan or 100.0) + 2.0)
        guncel_puan = kullanici.puan
        
    db.session.commit()
    return jsonify({
        "durum": "basarili", 
        "mesaj": "Ödeme başarıyla alındı.", 
        "guncel_puan": guncel_puan
    })
...
def musteri_dosya_sil():
    musteri_adi = request.json.get('musteri_adi')
    try:
...
@socketio.on('sayac_baslat')
def handle_sayac_baslat(data):
    global sayac_thread, sayac_aktif, mezat_durumu
    saniye = int(data.get('saniye', 30))
    
    sayac_aktif = False
    time.sleep(0.1)
    
    mezat_durumu['durum'] = 'Sayim'
    mezat_durumu['sure_bitis'] = time.time() + saniye
    
    sayac_thread = Thread(target=geri_sayim_gorevi, args=(saniye,))
    sayac_thread.start()

@socketio.on('satisi_bitir_server')
def handle_satis_bitir(data):
    global sayac_aktif, mezat_durumu, aktif_urun_id
    sayac_aktif = False
    
    urun_id = data.get('urun_id', aktif_urun_id)
    kazanan_ad = data.get('kazanan_ad', mezat_durumu['kazanan'])
    son_fiyat = data.get('son_fiyat', mezat_durumu['pey'])
    ...
    if urun_id:
        ...
        if urun:
            urun.durum = "Satıldı"
            urun.kazanan_adi = kazanan_ad
            db.session.commit()

    emit('urun_satildi_broadcast', {
        'urun_id': urun_id,
        'kazanan_ad': kazanan_ad,
        'son_fiyat': son_fiyat
    }, broadcast=True)

@socketio.on('musteri_onayla_server')
def handle_musteri_onay(data):
    kullanici_id = data.get('kullanici_id')
    kullanici = Kullanici.query.get(kullanici_id)
    if kullanici:
        kullanici.onayli_mi = True
        kullanici.sifre = "HLÇ" + str(kullanici_id) + "88"
        db.session.commit()
        emit('musteri_onaylandi_client', {
            'kullanici_id': kullanici_id, 
...
def init_db():
    with app.app_context():
        db.create_all()
        if Muzik.query.count() == 0:
            varsayilan_muzik = Muzik(url="https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3")
            db.session.add(varsayilan_muzik)
            db.session.commit()
