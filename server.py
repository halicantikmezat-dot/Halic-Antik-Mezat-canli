...
# --- KRİTİK DÜZELTME: SocketIO'yu CORS izinleriyle başlatıyoruz ---
...
aktif_urun = None
mezat_durumu = {"durum": "Bekliyor", "sure_bitis": 0, "pey": 0, "kazanan": "Yok"}

KATEGORILER = [
    "Mobilya ve ahşap ürünler",
    "Halı Kilim bez ve örtüler",
    "Obje ve Aksesuarlar",
    "Porselen dan Seramik",
    "Tablolar ve Resim",
...
def kayit_ol():
  ...
  for m in musteriler_db:
    if m["ad"] == ad:
      ...
      break
  if not bulundu:
    musteriler_db.append(
        {"ad": ad, "tel": tel, "mail": mail, "adres": adres, "bonus": 0}
    )

  # Bilgi güncellendiğinde tüm ekranlara haber ver
  socketio.emit("veri_guncellendi", broadcast=True)
...
def on_teklif_ver():
  ...
  on_teklifler_db = [
      t
      for t in on_teklifler_db
      if not (
          str(t["urun_id"]) == str(urun_id) and t["musteri_adi"] == musteri_adi
      )
  ]

  on_teklifler_db.insert(
      0,
      {
  ...
  # Ön teklif geldiğinde yönetici ve ekranlara anlık bildir
  socketio.emit("veri_guncellendi", broadcast=True)
  return jsonify(
      {"success": True, "mesaj": "Ön teklifiniz yöneticiye başarıyla iletildi."}
  )
...
def sahneye_al():
  ...
  urun_teklifleri = [
      t for t in on_teklifler_db if str(t["urun_id"]) == str(urun_id)
  ]
  if urun_teklifleri:
    en_yuksek_teklif = max(t["teklif"] for t in urun_teklifleri)
    en_yuksek_kisi = next(
        t["musteri_adi"]
        for t in urun_teklifleri
        if t["teklif"] == en_yuksek_teklif
    )
  ...
  # Sahneye ürün alındığında tüm telefonlara ve ekranlara sinyal gönder
  socketio.emit("veri_guncellendi", broadcast=True)
  return jsonify({"success": True})
...
def pey_ver():
  ...
  if not hedef_urun:
    return jsonify(
        {"success": False, "mesaj": "Ürün bulunamadı veya sahne aktif değil!"}
    )
  ...
  if islem == "hemen_al":
    ...
    mezat_durumu["pey"] = (
        hemen_al_fiyat if hemen_al_fiyat > 0 else hedef_urun.get("fiyat", 0)
    )
    ...
    musteri_pey_listesi = [
        ...
        for p in musteri_pey_listesi
        if not (
            str(p.get("urun_id")) == str(urun_id)
            and p.get("musteri_adi") == musteri_adi
        )
    ]

    musteri_pey_listesi.insert(
        0,
        {
            "urun_id": hedef_urun["id"],
            "lot": hedef_urun.get("lot"),
            ...
            "metin": (
                f"Lot #{hedef_urun['lot']} - {hedef_urun['ad']} ->"
                f" {musteri_adi} ({mezat_durumu['pey']} TL)"
            ),
        },
    )
    socketio.emit("veri_guncellendi", broadcast=True)
  ...
  if islem == "pey":
    mevcut_fiyat = (
        mezat_durumu["pey"]
        if aktif_urun and str(aktif_urun["id"]) == urun_id
        else hedef_urun.get("fiyat", 0)
    )
    if miktar <= mevcut_fiyat:
      return jsonify({
          "success": False,
          "mesaj": f"Teklif mevcut fiyatın ({mevcut_fiyat} TL) altında olamaz!",
      })
    ...
    uzgun_uyari = (
        eski_kazanan
        if eski_kazanan
        and eski_kazanan != "Yok"
        and eski_kazanan != musteri_adi
        else None
    )
...
def urun_ekle():
  ...
  for dosya in dosyalar:
    if dosya and dosya.filename:
      ...
      if dosya.filename.lower().endswith(
...
def musteri_dosya_sil():
  ...
  try:
    musteri_pey_listesi = [
        p for p in musteri_pey_listesi if p.get("musteri_adi") != musteri_adi
    ]
    socketio.emit("veri_guncellendi", broadcast=True)
    return jsonify({"success": True})
  except Exception as e:
...
if __name__ == "__main__":
  # Render üzerinde SocketIO ile çalıştırmak için socketio.run kullanılmalıdır
...
>>>>>>> b79f908ba801974fa211b0b05cb1b7a9dbbee02d
